...
function Hero() {
  ...
  return <section className="hero" id="top"><div className="hero-grain" /><div className="hero-meta"><span>Global access, reimagined</span><span className="scroll-cue">Scroll to explore <i /></span></div><motion.div className="hero-copy" initial="hidden" animate="visible" transition={{ staggerChildren: .13, delayChildren: .15 }}><motion.div variants={fadeUp}><Eyebrow light>A new kind of pathway</Eyebrow></motion.div><motion.h1 variants={fadeUp}>The world’s talent<br />deserves <em>a way in.</em></motion.h1><motion.p variants={fadeUp}>We’re developing the trusted infrastructure that connects extraordinary students, wherever they begin, with universities ready to invest in their potential.</motion.p><motion.div variants={fadeUp} className="hero-actions"><Button href="#universities" variant="lime">For universities</Button><a className="quiet-button" href="#model">Discover the model <ArrowDown size={17} /></a></motion.div></motion.div><motion.div style={{ y: artSpring }}><OrbitArt /></motion.div><div className="hero-foot">Independent by design. Global by default. <span>✦</span></div></section>;
}
